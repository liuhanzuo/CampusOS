 ...
 * 执行工具调用
 */
async function executeTool(helper, toolName, args) {
    try {
        switch (toolName) {
            case "get_schedule": {
                const result = await (0, thu_data_service_1.getScheduleInfo)(helper);
                return JSON.stringify(result);
            }
            case "get_sports_resources": {
                const date = parseRelativeDate(args.date) || args.date;
                const result = await (0, thu_data_service_1.getSportsResourceInfo)(helper, args.sport_name, date);
                return JSON.stringify(result);
            }
            case "get_report": {
                const result = await (0, thu_data_service_1.getReportInfo)(helper);
                return JSON.stringify(result);
            }
            case "get_card_info": {
                const result = await (0, thu_data_service_1.getCardInfo)(helper);
                return JSON.stringify(result);
            }
            case "get_electricity": {
                const result = await (0, thu_data_service_1.getElectricityInfo)(helper);
                return JSON.stringify(result);
            }
            case "get_library": {
                const result = await (0, thu_data_service_1.getLibraryInfo)(helper);
                return JSON.stringify(result);
            }
            case "get_news": {
                const result = await (0, thu_data_service_1.getNewsInfo)(helper, args.keyword);
                return JSON.stringify(result);
            }
            case "get_calendar": {
                const result = await (0, thu_data_service_1.getCalendarInfo)(helper);
                return JSON.stringify(result);
            }
            case "get_classroom": {
                const result = await (0, thu_data_service_1.getClassroomInfo)(helper, args.building, args.week);
                return JSON.stringify(result);
            }
            case "get_available_sports_venues": {
                return JSON.stringify({
                    success: true,
                    data: thu_data_service_1.sportsIdInfoList.map((v) => v.name),
                });
            }
            case "recharge_campus_card": {
                const result = await (0, thu_data_service_1.rechargeCardInfo)(helper, args.amount, args.pay_method || "wechat");
                return JSON.stringify(result);
            }
            default:
...
当前日期：${(0, dayjs_1.default)().format("YYYY年MM月DD日")}，${["日", "一", "二", "三", "四", "五", "六"][(0, dayjs_1.default)().day()]}。

请用中文回复，语气友好亲切。对于体育场馆查询，请重点标注哪些时段有空位可以预约。`;
/**
 * 与 GLM4-Flash 进行对话
 */
async function chat(helper, messages) {
    // 构建完整的消息列表
    const fullMessages = [
...
async function callGLM(messages) {
    ...
    const response = await (0, cross_fetch_1.default)(GLM_API_URL, {
        method: "POST",
        headers: {
